...
- Web Share를 우선하며 `canShare` 미지원이면 `share`를 시도한다. `share` 미지원, `canShare` false/예외 및 공유 실행 오류는 복사를 한 번 시도한다. `AbortError`는 취소로 조용히 종료하고 클립보드를 바꾸지 않는다. `InvalidStateError`는 재조작 안내만 제공한다.
- 실제 복사 성공 후에만 `링크를 복사했어요.`를 약 3초 표시한다. 시스템 공유 정상 종료에는 전달 완료 안내를 하지 않는다.
- 복사 실패 시 해당 화면의 비모달 패널에서 URL 전체 선택과 닫기를 제공한다. Clipboard API가 있으면 새 클릭으로 `링크 복사`를 재시도할 수 있다. 자동 재시도는 하지 않으며 닫기/Escape는 공유 버튼으로 포커스를 복구한다. history 항목은 추가하지 않는다.
