...
- E2E는 `e2e/` 아래에 둔다.
- `e2e/diary-share.spec.ts`는 공유 실패·취소, 복사 패널의 포커스·재시도·중복 실행 방지, 모달 공유 URL, 모바일 배치와 비공개 숨김을 검증한다. Web Share·Clipboard·일기 응답은 대역을 사용하므로 실제 OS 공유창과 수신자 열람 권한은 수동 검증 대상이다.
